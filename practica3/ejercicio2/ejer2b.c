...
int main(){
	...
	int  s;
	char c[NCARACTERES]; //Caracter que va a imprimir cada hilo
	//Declaracion de hilos
	pthread_t hilo[NHILOS];
	for(i=0;i<NHILOS;i++){
		c[i]=eligeCaracter(i);
		estadoHilo=pthread_create(&hilo[i], NULL, escribePantalla, (void*)&c[i]);
...
void *escribePantalla(void *args){
	...
	s=pthread_mutex_lock(&mtx); //bloquea el semaforo
	//printf("Hebra %ld, caracter a imprimir:%c\n",pthread_self(), *caracter);
	if(s!=0){
			perror("Error al cerrar el semaforo");
			printf("Error cerrando el semaforo :%d\n", errno);
			exit(EXIT_FAILURE);
