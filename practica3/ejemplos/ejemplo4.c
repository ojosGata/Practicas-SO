...
static void * threadFunc(void *arg){
	...
	for (j = 0; j < cnt; j++){
		...
		if (s != 0) printf("mutex_lock error...\n");
...
int main(int argc, char *argv[]){
	if(argc<=3){
		printf("Ussage: ./a.out Number_of_increasing_for_threadN Number_of_increasing_for_thread1 Number_of_increasing_for_thread2\n");
		exit(EXIT_FAILURE);
	}
	pthread_t tid;
	int s, j;
	...
	for (j = 1; j < argc; j++){
		...
		if (s != 0) printf("pthread_create error...\n");
	}
...
/*
The above code works, but it wastes CPU time, because the main thread continually loops, checking the state of the variable avail. A condition variable remedies this problem. It allows a thread to sleep (wait) until another thread notifies (signals) itcthat it must do something (i.e., that some “condition” has arisen that the sleeper must now respond
to).cA condition variable is always used in conjunction with a mutex. The mutexccprovides mutual exclusion for accessing the shared variable, while the conditioncvariable is used to signal changes in the variable’s state.
*/
